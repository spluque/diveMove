...
tdrX <- readTDR(ff, speed=TRUE, sep=";", na.strings="", as.is=TRUE)
png("plot_tdr.png"); plot(tdrX, interact=FALSE, cex.lab=1.3); dev.off()
@
\begin{figure}%[tbhp]
  \centering
  \includegraphics[width=\linewidth]{plot_tdr}
  \includegraphics[width=0.5\linewidth]{tcltk}
  \caption{The \Rmethod{plot()} method for \Rclass{TDR} objects produces an
    interactive plot of the data, allowing for zooming and panning.}
  \label{fig:1}
\end{figure}
...
a \Rfunction{plot()} method (Figure~\ref{fig:1}) for both \Rclass{TDR} and
\Rclass{TDRspeed} objects.  The \Rfunarg{interact} argument allows for
...
The \Rclass{TDRcalibrate} \Rmethod{plot()} method for these objects allows
visualizing the major wet/dry activities throughout the record
(Figure~\ref{fig:2}):

<<plot-gross-activity, eval=FALSE>>=
plot(dcalib, concurVars="light", concurVarTitles=c("speed (m/s)", "light"),
     surface=TRUE)
<<eval=TRUE, echo=FALSE, results=hide>>=
...
\begin{figure}[tb]
  ...
  \caption{The \Rfunction{plot()} method for \Rclass{TDRcalibrate} objects
    displays information on the major activities identified throughout the
    record (wet/dry periods here).}
  \label{fig:2}
\end{figure}
...
\Rmethod{plot()} method.  This is useful if, for instance, closer
...
plot(dcalib, diveNo=2:8, labels="dive.phase")
<<eval=TRUE, echo=FALSE, results=hide>>=
png("plot_tdr_dives.png", height=380)
plot(dcalib, diveNo=2:8, labels="dive.phase", interact=FALSE,
...
\begin{figure}%[tb]
  ...
  \caption{The \Rmethod{plot()} method for \Rclass{TDRcalibrate} objects
